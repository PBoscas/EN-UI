    ...
    e.addModule(function(t, s) {
        ...
        t.extend(i.prototype, {
            ...
            initStructure: function() {
                this.win = t(s), this.doc = t(document), this.realElement = t(this.options.element), 
                this.fakeElement = t(this.options.fakeAreaStructure).insertAfter(this.realElement), 
                this.selectTextContainer = this.fakeElement.find(this.options.selectTextSelector), 
                this.selectText = t("<span></span>").appendTo(this.selectTextContainer), h(this.fakeElement), 
                ...
                this.fakeElement.attr("title", this.realElement.attr("title")), this.fakeDropTarget = this.options.fakeDropInBody ? t("body") : this.fakeElement);
            },
            ...
            onSelectAreaPress: function(e) {
                !this.options.fakeDropInBody && t(e.target).closest(this.dropdown).length || 1 < e.button || this.realElement.is(":disabled") || (this.selectOpenedByEvent = e.pointerType, 
                this.toggleDropdown(), this.focusedFlag || ("mouse" === e.pointerType ? this.realElement.focus() : this.onFocus(e)), 
            ...
            createDropdown: function() {
                this.dropdown && (this.list.destroy(), this.dropdown.remove()), this.dropdown = t(this.options.fakeDropStructure).appendTo(this.fakeDropTarget), 
                this.dropdown.addClass(r(this.realElement.prop("className"), this.options.selectClassPrefix)), 
                h(this.dropdown), this.realElement.prop("multiple") && this.dropdown.addClass("jcf-compact-multiple"), 
            ...
            repositionDropdown: function() {
                var t, i = this.fakeElement.offset(), c = this.fakeElement[0].getBoundingClientRect(), n = c.width || c.right - c.left, l = this.fakeElement.outerHeight(), r = this.dropdown.css("width", n).outerHeight(), e = this.win.scrollTop(), a = this.win.height(), c = !1;
                i.top + l + r > e + a && i.top - r > e && (c = !0), this.options.fakeDropInBody && (e = "static" !== this.fakeDropTarget.css("position") ? this.fakeDropTarget.offset().top : 0, 
                e = this.options.flipDropToFit && c ? (t = i.left, i.top - r - e) : (t = i.left, 
                i.top + l - e), this.dropdown.css({
                    width: n,
                    left: t,
                    top: e
            ...
            showDropdown: function() {
                ...
                this.list.refresh(), this.win.on("resize", this.onResize), this.doc.on("jcf-pointerdown", this.onOutsideClick));
            },
            ...
            refreshSelectedText: function() {
                var e = this.realElement.prop("selectedIndex"), i = this.realElement.prop("options")[e], o = i ? i.getAttribute("data-image") : null, n = "";
                this.realElement.prop("multiple") ? (t.each(this.realElement.prop("options"), function(e, t) {
                    t.selected && (n += (n ? ", " : "") + t.innerHTML);
                }), n = n || (this.realElement.attr("placeholder") || ""), this.selectText.removeAttr("class").html(n)) : i ? this.currentSelectedText === i.innerHTML && this.currentSelectedImage === o || (e = r(i.className, this.options.optionClassPrefix), 
                this.selectText.attr("class", e).html(i.innerHTML), o ? (this.selectImage || (this.selectImage = t("<img>").prependTo(this.selectTextContainer).hide()), 
                this.selectImage.attr("src", o).show()) : this.selectImage && this.selectImage.hide(), 
                this.currentSelectedText = i.innerHTML, this.currentSelectedImage = o) : (this.selectImage && this.selectImage.hide(), 
                this.selectText.removeAttr("class").empty());
            },
            refresh: function() {
                "none" === this.realElement.prop("style").display ? this.fakeElement.hide() : this.fakeElement.show(), 
                this.refreshSelectedText(), this.fakeElement.toggleClass(this.options.disabledClass, this.realElement.is(":disabled"));
            },
            destroy: function() {
                ...
                this.fakeElement.remove(), this.doc.off("jcf-pointerup", this.onSelectAreaRelease), 
            ...
            initStructure: function() {
                this.realElement = t(this.options.element), this.fakeElement = t(this.options.fakeStructure).insertAfter(this.realElement), 
                this.listHolder = this.fakeElement.find(this.options.listHolder), h(this.fakeElement), 
                this.fakeElement.addClass(r(this.realElement.prop("className"), this.options.selectClassPrefix)), 
                this.realElement.addClass(this.options.hiddenClass), this.list = new n({
                    useCustomScroll: this.options.useCustomScroll,
            ...
            initStructure: function() {
                this.element = t(this.options.element), this.indexSelector = "[" + this.options.indexAttribute + "]", 
                ...
                this.rebuildList(), this.element.prop("multiple") && (this.previousSelection = this.getSelectedOptionsIndexes());
            },
            attachEvents: function() {
            ...
            onHoverItem: function(t) {
                t = parseFloat(t.currentTarget.getAttribute(this.options.indexAttribute));
            ...
            onSelectItem: function(e) {
                ...
                e.shiftKey || (this.lastClickedIndex = i), this.refreshSelectedClass(), "mouse" === o && this.scrollToActiveOption(), 
                t(this).trigger("select"));
            },
            rebuildList: function() {
                var s = this, l = this.element[0];
                this.storedSelectHTML = l.innerHTML, this.optionIndex = 0, this.list = t(this.createOptionsList(l)), 
                this.listHolder.empty().append(this.list), this.realOptions = this.element.find("option"), 
                this.fakeOptions = this.list.find(this.indexSelector), this.fakeListItems = this.list.find("." + this.options.captionClass + "," + this.indexSelector), 
                delete this.optionIndex;
                var o = this.options.maxVisibleItems, l = this.element.prop("size");
                1 < l && !this.element.is("[jcf-size]") && (o = l);
                l = this.fakeOptions.length > o;
            ...
            refreshSelectedClass: function() {
                var t = this, s = this.element.prop("multiple"), e = this.element.prop("selectedIndex");
                s ? this.realOptions.each(function(e, s) {
                    t.fakeOptions.eq(e).toggleClass(t.options.selectedClass, !!s.selected);
                }) : (this.fakeOptions.removeClass(this.options.selectedClass + " " + this.options.hoverClass), 
                e = this.fakeOptions.eq(e).addClass(this.options.selectedClass), this.options.useHoverClass && e.addClass(this.options.hoverClass));
            },
            ...
            getActiveOptionOffset: function() {
                var o = this.getChangedSelectedIndex();
                if (null !== o) {
                    var t = this.listHolder.height(), s = this.listHolder.prop("scrollTop"), n = this.fakeOptions.eq(o), o = n.offset().top - this.list.offset().top, n = n.innerHeight();
                    return s + t <= o + n ? o - t + n : o < s ? o : void 0;
                }
            },
            getOverflowHeight: function(s) {
                var t = this.fakeListItems.eq(s - 1), s = this.list.offset().top;
            ...
            createOption: function(e) {
                ...
                return i && ((s = document.createElement("img")).src = i, t.insertBefore(s, t.childNodes[0])), 
                e.disabled && (t.className += " " + this.options.disabledClass), e.className && (t.className += " " + r(e.className, this.options.cloneClassPrefix)), 
                t;
            },
            createOptGroup: function(s) {
                var i = document.createElement("span"), o = s.getAttribute("label"), t = document.createElement("span");
                return t.className = this.options.captionClass, t.innerHTML = o, i.appendChild(t), 
                s.children.length && (s = this.createOptionsList(s), i.appendChild(s)), i.className = this.options.groupClass, 
                i;
            },
